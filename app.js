...
const tabButtons = document.querySelectorAll('.tree-item');
const sections = document.querySelectorAll('.editor-panel');
...
const editorActiveTabTitle = document.getElementById('editor-active-tab-title');
...
async function loadProjectsData() {
  try {
    ...
    projectsGrid.innerHTML = `
      <div style="grid-column: 1/-1; padding: 20px; font-family: var(--font-mono); font-size: 0.82rem; text-align: center; color: var(--accent-red); border: 1px solid var(--border-color); border-radius: 6px; background: rgba(239, 68, 68, 0.05);">
        <p>// Erro: Não foi possível ler o arquivo projects.json.</p>
      </div>
    `;
  }
}
...
function switchTab(tabId) {
  ...
  // Update tab title in editor header
  if (editorActiveTabTitle) {
    if (tabId === 'resumo') editorActiveTabTitle.textContent = 'about.md';
    else if (tabId === 'trabalhos') editorActiveTabTitle.textContent = 'projects.json';
    else if (tabId === 'detalhes') editorActiveTabTitle.textContent = 'inspect.log';
  }

  // Switch active section panels
...
function updateExpanderUIState() {
  if (utilitiesExpanded) {
    ...
    btnToggleUtilities.querySelector('span').textContent = 'Ocultar Projetos Utilitários';
...
function renderProjects() {
  ...
  if (filtered.length === 0) {
    projectsGrid.innerHTML = '<p style="grid-column: 1/-1; text-align: center; font-family: var(--font-mono); font-size: 0.82rem; color: var(--text-muted); padding: 40px;">// Nenhum registro encontrado nesta categoria.</p>';
  ...
  filtered.forEach(project => {
    ...
    // Build techs HTML (Fira code tags)
    ...
    card.innerHTML = `
      <div class="project-card-header">
        ...
        <div class="project-card-links">
          ${project.githubUrl ? `<a href="${project.githubUrl}" target="_blank" rel="noopener noreferrer" title="GitHub Repo">${githubIconSvg}</a>` : ''}
          ${project.demoUrl ? `<a href="${project.demoUrl}" target="_blank" rel="noopener noreferrer" title="Live Demo">${externalLinkSvg}</a>` : ''}
        </div>
      </div>
      ...
      <div class="project-card-footer">
        ...
        <div class="project-action-row">
          <button class="btn-inspect-project" data-id="${project.id}">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="12" x2="19" y2="12"></line><polyline points="12 5 19 12 12 19"></polyline></svg>
          </button>
        </div>
      </div>
    `;
...
function translateCategory(cat) {
  switch(cat) {
    case 'principal': return 'featured';
    case 'secundario': return 'commercial';
    case 'utilitario': return 'utility';
    default: return cat;
  }
}
...
function viewProjectDetails(projectId) {
  ...
  if (project.category === 'principal') {
    detailCategoryBadge.style.color = 'var(--accent-green)';
    detailCategoryBadge.style.borderColor = 'var(--border-color)';
  } else if (project.category === 'secundario') {
    detailCategoryBadge.style.color = 'var(--accent-blue)';
    detailCategoryBadge.style.borderColor = 'var(--border-color)';
  } else {
  ...
  detailIntegration.innerHTML = project.integration || 'Nenhuma integração especial listada.';
